...
#else
char dns_txt_record[] = "v=DKIM1; p=MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQDmhEcoXX2sIZNUPYsq6MA23RgfuAkup160yZeZzCPwL8pgaA2+COqwQU1MgZMPwxSWXNFtrUMNNok+Uz4jsY828yI80/dgXU8WIHr1tmnDk5QKm1b+QbgcWq1ZHQYG3aZC4WUww+N/8npWr8uG7tca2Vt/LCcLTK1zwEGeyP9ePwIDAQAB;";
#endif
...
int main_test_verify(/*int argc, char *argv[]*/) {
 // FILE *input;
 // FILE *debug=0;
...
//  pdkim_set_debug_stream(ctx,debug);
...
#else
  int rv=read_entire_log_file(buffer, MAX_LINE_LEN);
  if (pdkim_feed(ctx, buffer, strlen(buffer)) != PDKIM_OK) {
	  printf("pdkim_feed() error\n");
	  goto BAIL;
  }
  ...
  if (pdkim_feed_finish(ctx,&signatures) == PDKIM_OK) {
    while (signatures != NULL) {
      ...
      /* Try next signature */
      signatures = (pdkim_signature *)signatures->next;
    }
  }
