...
#include "./my_lib/my_lib.h"    // mkr_my_strdup  -> NOT SURE WHY IT CAUSES PROBLEM
//#define mkr_my_strdup strdup

/*svk  
I used stricmp and strnicmp, instead of strcasecmp and strnicmp, and included the head file string.h,
*/
...
pdkim_stringlist *pdkim_append_stringlist(pdkim_stringlist *base, char *str) {
  ...
  new_entry->value = mkr_my_strdup(str);
      ...
      if ( (*p == ';') || (*p == '\0') ) {
        if (cur_tag->len > 0) {
          ...
          switch (cur_tag->str[0]) {
            ...
            break;
            case 's':
              sig->selector = mkr_my_strdup(cur_val->str);
            break;
            case 'd':
              sig->domain = mkr_my_strdup(cur_val->str);
            break;
            case 'i':
            ...
            break;
            case 'h':
              sig->headernames = mkr_my_strdup(cur_val->str);
            break;
            case 'z':
...
pdkim_pubkey *pdkim_parse_pubkey_record(pdkim_ctx *ctx, char *raw_record) {
      ...
      if ( (*p == ';') || (*p == '\0') ) {
        if (cur_tag->len > 0) {
          ...
          switch (cur_tag->str[0]) {
            ...
            break;
            case 'h':
              pub->hashes = mkr_my_strdup(cur_val->str);
            break;
            case 'g':
              pub->granularity = mkr_my_strdup(cur_val->str);
            break;
            case 'n':
            ...
            break;
            case 's':
              pub->srvtype = mkr_my_strdup(cur_val->str);
            break;
            case 't':
  ...
  if (pub->version     == NULL) pub->version     = mkr_my_strdup(PDKIM_PUB_RECORD_VERSION);
  if (pub->granularity == NULL) pub->granularity = mkr_my_strdup("*");
  if (pub->keytype     == NULL) pub->keytype     = mkr_my_strdup("rsa");
  if (pub->srvtype     == NULL) pub->srvtype     = mkr_my_strdup("*");
...
int pdkim_header_complete(pdkim_ctx *ctx) {
  ...
  if (ctx->mode == PDKIM_MODE_VERIFY) {
    if (strnicmp(ctx->cur_header->str,
...
DLLEXPORT int pdkim_feed_finish(pdkim_ctx *ctx, pdkim_signature **return_signatures) {
  ...
  while (sig != NULL) {
    ...
    if (ctx->mode == PDKIM_MODE_SIGN) {
      ...
      while (p != NULL) {
        ...
        if (sig->canon_headers == PDKIM_CANON_RELAXED)
          ...
          rh = mkr_my_strdup(p->value);               /* just copy it for simple canon */
    ...
    /* When verifying, walk through the header name list in the h= parameter and
      ...
      char *b = mkr_my_strdup(sig->headernames);
      char *p = b;
      char *q = NULL;
      ...
      while(1) {
        ...
        while (hdrs != NULL) {
          if ( (hdrs->tag == 0) &&
               (strnicmp(hdrs->value,p,strlen(p)) == 0) &&
               ((hdrs->value)[strlen(p)] == ':') ) {
            char *rh = NULL;
            if (sig->canon_headers == PDKIM_CANON_RELAXED)
              rh = pdkim_relax_header(hdrs->value,1); /* cook header for relaxed canon */
            else
              rh = mkr_my_strdup(hdrs->value);               /* just copy it for simple canon */
            if (rh == NULL) return PDKIM_ERR_OOM;
            /* Feed header to the hash algorithm */
            if (sig->algo == PDKIM_ALGO_RSA_SHA1)
              sha1_update(&(sha1_headers),(unsigned char *)rh,strlen(rh));
    ...
    if (ctx->mode == PDKIM_MODE_SIGN) {
      ...
      sig->headernames = mkr_my_strdup(headernames->str);
    ...
    /* VERIFICATION ----------------------------------------------------------- */
      ...
      sig_hdr = mkr_my_strdup(sig->rawsig_no_b_val);
    }
    /* ------------------------------------------------------------------------ */
...
DLLEXPORT pdkim_ctx *pdkim_init_sign(int input_mode,
  ...
  ctx->sig->domain = mkr_my_strdup(domain);
  ctx->sig->selector = mkr_my_strdup(selector);
  ctx->sig->rsa_privkey = mkr_my_strdup(rsa_privkey);
...
DLLEXPORT int pdkim_set_optional(pdkim_ctx *ctx,
  ...
  if (identity != NULL) {
    ctx->sig->identity = mkr_my_strdup(identity);
    if (ctx->sig->identity == NULL) return PDKIM_ERR_OOM;
  }

  if (sign_headers != NULL) {
    ctx->sig->sign_headers = mkr_my_strdup(sign_headers);
