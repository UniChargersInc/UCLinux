...
#define SMTP_DEBUG 0
#if SMTP_DEBUG
#define M_PRINTF m_printf
void M_OutputDebugStringA(const char *title,char *msg)
{
	M_PRINTF("%s : %s",title, msg);
}
#define My_OutputDebugStringA M_OutputDebugStringA
#else
#define My_OutputDebugStringA
#define M_PRINTF
#endif
...
CSmtp::CSmtp()
{
	...
	m_sLocalHostName = "h2evon.ca"; //hostname;
...
void CSmtp::Send(string &msg)
{
	...
	if(hSocket==INVALID_SOCKET)
	{
		bool rv = false;
		try {
			rv = ConnectRemoteServer(m_sSMTPSrvName.c_str(), m_iSMTPSrvPort, m_type, m_bAuthenticate);
		}
		catch (ECSmtp e)
		{
			M_PRINTF("Conn Ex: err:%d \r\n",e.GetErrorNum());
			throw;
		}
	...
	try{
		...
		//m_printf("Sending command_RCPTTO %d %d %d", Recipients.size(), CCRecipients.size(), BCCRecipients.size());
		for(i=0;i<Recipients.size();i++)
		{
			snprintf(SendBuf, BUFFER_SIZE, "RCPT TO:<%s>\r\n", (Recipients.at(i).Mail).c_str());
			//m_printf("Sending command_RCPTTO buf: %s", SendBuf);
			int rv1=SendData(pEntry);
			//m_printf("Sending command_RCPTTO rv: %d", rv1);
			ReceiveResponse(pEntry);
		}
		...
		M_PRINTF("***** FindCommandEntry command_DATA\r\n");
		...
		int len = msg.size(),sent=0;
		while (len>0)
		{
			if (len < BUFFER_SIZE-1)
			{
				//snprintf(SendBuf, BUFFER_SIZE, "%s\r\n", GetMsgLineText(i));
				memcpy(SendBuf, pStr+sent, len);
				SendBuf[len] = 0;
				sent += len;
			}
			else
			{
				memcpy(SendBuf, pStr+sent, BUFFER_SIZE-1);
				SendBuf[BUFFER_SIZE-1] = 0;   //SendBuf[len] = 0;                               // !!!!!!!!!!!!!!!!!!
				sent += (BUFFER_SIZE-1);
			}
			len -= sent;
			SendData(pEntry);
		...
		for(FileId=0;FileId<Attachments.size();FileId++)
		{
			const char *pFileName = Attachments[FileId].c_str();
			
		
			snprintf(SendBuf, BUFFER_SIZE, "--%s\r\n",BOUNDARY_TEXT);
			strcat(SendBuf,"Content-Type: application/x-msdownload; name=\"");
#ifndef MKR_LINUX
			strcat(SendBuf,/*&FileName[Attachments[FileId].find_last_of("/") + 1]*/pFileName);
#else
			strcat(SendBuf,/*&FileName[Attachments[FileId].find_last_of("/") + 1]*/pFileName);
#endif
			strcat(SendBuf,"\"\r\n");
			...
			// opening the file:
			char filename_s_buf2[128], FileBuf[54];
			snprintf(filename_s_buf2, 128, "%s%s", REPORT_GEN_FILES, pFileName);
			M_PRINTF("Attachment file:%s \r\n", filename_s_buf2);
			hFile = fopen(filename_s_buf2,"rb");
			if(hFile == NULL)
				throw ECSmtp(ECSmtp::FILE_NOT_EXIST);
		...
		M_PRINTF("***** FindCommandEntry _DATA END\r\n");
								...
								const char* password/*=NULL*/)
{
...
#else
		if(ioctlsocket(hSocket,FIONBIO, (unsigned long*)&ul) == SOCKET_ERROR)  // (ul=1) enable non-blocking io
#endif
		{
#ifdef MKR_LINUX
			close(hSocket);
	...
	catch(const ECSmtp&)
	{
		if(RecvBuf[0]=='5' && RecvBuf[1]=='3' && RecvBuf[2]=='0')
			m_bConnected=false;
		DisconnectRemoteServer();
		throw;  //the statement throw; means "re-throw the current exception."
		return false;
	}
...
int CSmtp::SendData(Command_Entry* pEntry)
{
	int rv = 0;
	if(m_ssl != NULL)
	{
	...
	//My_OutputDebugStringA("SendD",SendBuf);
	FD_CLR(hSocket,&fdwrite);
	return rv;
}
...
{
	m_sLogin = Login==0?"": Login;
}


void CSmtp::SetPassword(const char *Password)
{
	m_sPassword = Password==0?"": Password;
}
...
void CSmtp::ReceiveData_SSL(SSL* ssl, Command_Entry* pEntry)
{
	...
	//My_OutputDebugStringA("RecvData_SSL: ", SendBuf);
}
...
void CSmtp::ReceiveResponse(Command_Entry* pEntry)
{
	...
	//My_OutputDebugStringA("RecvR: ",RecvBuf);
	//m_printf("Recv:%s %d %d", line.c_str(), reply_code, pEntry->error);
	if(reply_code != pEntry->valid_reply_code)
	{
		throw ECSmtp(pEntry->error,"RecvR");  // e.g error=309
	}
}
...
void CSmtp::SendData_SSL(SSL* ssl, Command_Entry* pEntry)
{
	...
	My_OutputDebugStringA("SendData_SSL: ",SendBuf);
	FD_ZERO(&fdwrite);
	FD_ZERO(&fdread);
}
//
int init_ssl_certificates(SSL_CTX *ctx);
...
#if 0
//svk  Init Certificates
int init_ssl_certificates(SSL_CTX *ssl_ctx)
{
	const char *tls_cafile = "path to file";
	const char *tls_certfile = "path to cert";
	const char *tls_keyfile = "path to key";
	int ret = SSL_CTX_load_verify_locations(ssl_ctx, tls_cafile, 0);
	if (ret == 0) {
	...
	if (tls_certfile) {
		ret = SSL_CTX_use_certificate_chain_file(ssl_ctx, tls_certfile);
		if (ret != 1) {
	...
	if (tls_keyfile) {
		ret = SSL_CTX_use_PrivateKey_file(ssl_ctx, tls_keyfile, SSL_FILETYPE_PEM);
		if (ret != 1) {
			//Error: Unable to load client key file
			return -3;
		}
...
int init_ssl_certificates(SSL_CTX *ssl_ctx)
{
	const char *tls_cafile = 0;
	const char *tls_certfile = mkr_mbedtls_x509_crt_cert_file;
	const char *tls_keyfile = mkr_mbedtls_pk_cert_keyfile;
	int ret = SSL_CTX_load_verify_locations(ssl_ctx, tls_cafile, 0);
	SSL_CTX_set_verify(ssl_ctx, 0, 0);

	SSL_CTX_use_certificate_file(ssl_ctx, /*cfg->pemfile*/tls_certfile, SSL_FILETYPE_PEM);
	SSL_CTX_use_PrivateKey_file(ssl_ctx, tls_keyfile, SSL_FILETYPE_PEM);
	return 1;
}
#endif