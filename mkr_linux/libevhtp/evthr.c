...
//for WIN32 I'm not sure that these functions are exact equivalent ????????
#define atomic_inc(ptr) InterlockedIncrement ((ptr))
#define atomic_dec(ptr)  InterlockedDecrement((ptr))
#define atomic_add(ptr,v)  InterlockedExchangeAdd((ptr),(v))
#else
...
evthr_inc_backlog(evthr_t * evthr) {
	atomic_inc(&evthr->cur_backlog);// __sync_fetch_and_add(&evthr->cur_backlog, 1);
}

inline void
evthr_dec_backlog(evthr_t * evthr) {
	atomic_dec(&evthr->cur_backlog);//__sync_fetch_and_sub(&evthr->cur_backlog, 1);
}

inline int
evthr_get_backlog(evthr_t * evthr) {
	return atomic_add(&evthr->cur_backlog,0);//__sync_add_and_fetch(&evthr->cur_backlog, 0);
}
...
_evthr_loop(void * args) {
    ...
    if (thread->err == 1) {
        //fprintf(stderr, "FATAL ERROR!\n");    // svk we have this error the process terminates !!
		//void event_warnx(const char *fmt, ...)
		event_warnx("<FATAL ERROR>!\n");
    }
