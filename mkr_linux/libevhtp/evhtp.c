...
_evhtp_connection_accept(evbase_t * evbase, evhtp_connection_t * connection) {
    ...
    if (connection->htp->ssl_ctx != NULL) {
		...
		//m_printf("catch bev=%p ssl=%p\r\n", connection->bev, connection->ssl);
        goto end;
    }
#endif
...
#ifndef WIN32
    signal(SIGPIPE, /*SIG_IGN*/signal_callback_handler);
#endif
...
evhtp_connection_free(evhtp_connection_t * connection) {
    ...
    if (connection->bev) {
        ...
        if (connection->ssl != NULL) {
          ...
          //  SSL_shutdown(connection->ssl);    // svk OpenSSL SSL_shutdown received signal SIGPIPE, Broken pipe !!!
			//the below workaround did not help !!!
			/*int iRet = SSL_get_shutdown(connection->ssl);
			if (iRet >= 0) 
