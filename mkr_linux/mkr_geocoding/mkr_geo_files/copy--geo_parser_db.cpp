...
int GeoDbCls::readGeometryInfo(const std::vector<std::string>& req,/*GeoResponse& gResp*/MkrGeoCoderResponseResult &result)
{
	...
	std::string sql = "SELECT name,rowid FROM names WHERE name like '%"+req[0]+"%' ;";
	db << sql >> [&](std::string strName, uint64_t nameid) {
...
int GeoDbCls::readGeometryInfo(std::vector<uint64_t> nameidV, uint64_t isinid, int type, std::vector<GeometryInfo>& geomInfos,int numberOfRows)
{
	...
	db << sql >> [&](uint64_t entid,uint64_t nameid, std::vector<char> OutV) {
		...
		else if (type == EntityType::ADMIN_BOUNDARIES || type==EntityType::STREET_BOUNDARIES || type == EntityType::BUILDING_BOUNDARIES || type == EntityType::GEO_PLACE_BOUNDARIES)
		{
