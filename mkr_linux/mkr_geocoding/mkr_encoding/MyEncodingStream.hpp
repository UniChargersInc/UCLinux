...
#ifndef MY_NKR_GEOCODING_ENCODINGSTREAM_H_
#define MY_NKR_GEOCODING_ENCODINGSTREAM_H_

#include <string>
#include <stdexcept>
#include <cglib/vec.h>

#include "var_length_encording.hpp"

namespace mkr_geocoding { 
    class MyEncodingStream final {
    public:
        explicit MyEncodingStream(const void* data, std::size_t size) : _data(reinterpret_cast<const unsigned char*>(data)), _size(size) { }
        ...
        T readNumber() {
			...
			/*
			std::size_t maxSize = _size - _offset;
			
				uint64_t v1 = decodeVarint<T>(&_data[_offset], maxSize > 0? maxSize:0);
				int64_t val = zz_unmap<int64_t, uint64_t>(v1);
			...
			*/
			...
			return static_cast<T>(((ret >> 1) ^ -(ret & 0x1)));
        }
