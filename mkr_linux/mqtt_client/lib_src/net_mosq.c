...
#  endif
	...
	/*if(mosq->in_callback == false && mosq->threaded == false){
		printf("svk-_mosquitto_packet_queue..\r\n");
		return _mosquitto_packet_write(mosq);                                // svk -> ?? the packet will be sent in _mosquitto_loop_write 
	}else*/
	{
...
int _mosquitto_socket_connect(struct mosquitto *mosq, const char *host, uint16_t port, const char *bind_address, bool blocking)
{
	...
	if(mosq->tls_cafile || mosq->tls_capath || mosq->tls_psk){
		...
		if(!mosq->ssl){
			COMPAT_CLOSE(sock);
			//printf("svk- SSL INIT..ERROR\r\n");
			return MOSQ_ERR_TLS;
		}
		//printf("svk- SSL INIT_1..\r\n");
		SSL_set_ex_data(mosq->ssl, tls_ex_index_mosq, mosq);
		bio = BIO_new_socket(sock, BIO_NOCLOSE);
		if(!bio){
			COMPAT_CLOSE(sock);
			//printf("svk- SSL Bio..ERROR\r\n");
			return MOSQ_ERR_TLS;
		}
		SSL_set_bio(mosq->ssl, bio, bio);
		//printf("svk- SSL INIT_2..\r\n");
		mosq->sock = sock;
		if(mosquitto__socket_connect_tls(mosq)){
			//printf("svk- SSL Connect..ERROR\r\n");
			return MOSQ_ERR_TLS;
		}
...
#ifndef WIN32
		int rv= write(mosq->sock, buf, count);
		//printf("svk-write_2 count:%d rv:%d\r\n",count, rv);
		return rv;
#else
