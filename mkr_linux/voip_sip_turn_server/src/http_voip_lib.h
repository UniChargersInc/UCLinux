...
template <typename T>
inline bool write_headers(Stream& strm, const T& res)
{
...
inline void write_response(Stream& strm, const Request& req, const Response& res)
{
	...
	socket_printf(strm, "SIP/2.0 %s\r\n", res.statusStr);

    bool rv=write_headers(strm, res);
    if (rv && !res.body.empty()) {
...
inline bool HttpServer::read_request_line(Stream& strm, Request& req)
{
    ...
    static std::regex re("(REGISTER|PRACK|INVITE|ACK|BYE|CANCEL|UPDATE|INFO|SUBSCRIBE|NOTIFY|REFER|MESSAGE|OPTIONS) ([^?]+)(?:\\?(.+?))? SIP/2\\.[01]\r\n");
