...
public:
   ...
   // bool set_base_dir(const char* path);
    ...
    bool listen(const char* host, int port, int *terminate, int socket_flags = 0);
    void stop();
...
public:
    SSLHttpServer(const char* cert_path, const char* private_key_path);
    virtual ~SSLHttpServer();
...
socket_t create_socket(const char* host, int port, Fn fn, int socket_flags = 0)
{
    ...
    if (getaddrinfo(host, service.c_str(), &hints, &result)) {
		//m_printf("create_socket..fail_1\r\n");
		return -1;
    }

    for (auto rp = result; rp; rp = rp->ai_next) {

		struct sockaddr_in *pA = (struct sockaddr_in *)rp->ai_addr;
		const char *ip = inet_ntoa(pA->sin_addr);
...
inline socket_t create_server_socket(const char* host, int port, int socket_flags)
{
   ...
   //     if (::bind(sock, ai.ai_addr, ai.ai_addrlen)) {
			//struct sockaddr_in *pA = (struct sockaddr_in *)&ai.ai_addr;
			//const char *ip=inet_ntoa( pA->sin_addr);

			//printf("could not bind..ip=%s port=%d errno=%d\r\n",ip,pA->sin_port,errno);
   //           return false;
   //     }
   ...
   // }, socket_flags);
...
#ifndef WIN32
	if (setsockopt(_sockfd, SOL_SOCKET, SO_REUSEPORT, &yes, sizeof(int)) < 0)
	{
		M_HTTP_PRINTF("http ERROR on SO_REUSEPORT  listening socket..errn=%d\r\n", (int)errno);
	}
#endif
	...
	if (::bind(_sockfd, (struct sockaddr *)&_addr, sizeof(_addr)) == -1) {
		//printf("::bind false.. sock=%d host=%s port=%d errno=%d\r\n",_sockfd,host,port, errno);
		if (errno == EACCES)
		{
		}
		return -1;
	...
	if (listen(_sockfd, 5)) { // Listen through 5 channels
			//printf("listen false..\r\n");
            return -1;
     }
...
inline void write_headers(Stream& strm, const T& res)
{
	...
	m_printf("Content-Type: %s\r\n", t);
    socket_printf(strm, "Content-Length: %ld\r\n", res.body.size());
	m_printf("Content-Length: %ld\r\n\r\n", res.body.size());
    strm.write("\r\n");
}
...
inline void write_response(Stream& strm, const Request& req, const Response& res)
{
	...
	socket_printf(strm, "HTTP/1.0 %d\r\n", res.status);
	m_printf("Resp:HTTP/1.0 %d method:%s bsize:%d\r\n", res.status, req.method.c_str(), res.body.size());
    write_headers(strm, res);

    //if (!res.body.empty() && req.method != "HEAD") 
	if (res.body.size()>0 /*&& req.method != "HEAD"*/)
	{
        strm.write(res.body.data(), res.body.size());
		m_printf("bodysize: %d\r\n", res.body.size());
    }
}
...
inline std::string Request::get_param(const char* key) const
{
...
inline void HttpServer::get(const char* pattern, Handler handler)
{
    //get_handlers_.push_back(std::make_pair(std::regex(pattern), handler));
	get_handler_ = handler;
}

inline void HttpServer::post(const char* pattern, Handler handler)
{
    //post_handlers_.push_back(std::make_pair(std::regex(pattern), handler));
	post_handler_ = handler;
}

//inline bool HttpServer::set_base_dir(const char* path)
//{
...
inline void HttpServer::set_logger(Logger logger)
{
    logger_ = logger;
}

//===================SSL Thread specifics=============================================
#if USE_THREAD_POOL && defined(CPPHTTPLIB_OPENSSL_SUPPORT)
...
static void init_locks(void)
{
	...
	CRYPTO_set_locking_callback(lock_callback);//CRYPTO_set_locking_callback((void(*)())lock_callback);
}
...
#endif
//===============================================================
inline bool HttpServer::listen(const char* host, int port,int *terminate, int socket_flags)
{
    svr_sock_ = detail::create_server_socket(host, port, socket_flags);
    if (svr_sock_ == -1) {
		M_HTTP_PRINTF("Could not create socket\r\n");
		return false;
    }
    ...
    while (*terminate==1) {
		...
		//M_HTTP_PRINTF("socket accepted..\r\n");
        if (sock == -1) {
            if (svr_sock_ != -1) {
                detail::close_socket(svr_sock_);
                ret = false;
				M_HTTP_PRINTF("socket accepted..failed\r\n");
            } else {
                 M_HTTP_PRINTF("The server socket was closed by user.\r\n");
            }
            break;
        }

		//M_HTTP_PRINTF("proceed read_and_close_socket. sock=%d\r\n",sock);
#if !USE_THREAD_POOL
...
#endif
#else
		//auto result = pool.submit([this](int sock) 
		//	{ 
		//		//M_HTTP_PRINTF("this->read_and_close_socket_1. sock=%d\r\n",sock);
		//		bool rv = this->read_and_close_socket(sock);
		//		
		//		return rv; 
		//	}, sock);
	...
	M_HTTP_PRINTF("The server pool.shutdown()..\r\n");
#if USE_THREAD_POOL
...
inline bool HttpServer::read_request_line(Stream& strm, Request& req)
{
...
inline bool HttpServer::routing(Request& req, Response& res)
{
  /*  if (req.method == "GET" && handle_file_request(req, res)) {
    ...
    if (req.method == "GET" || req.method == "HEAD") {
       // return dispatch_request(req, res, get_handlers_);
		get_handler_(req, res);
		return true;
    } else if (req.method == "POST") {
       // return dispatch_request(req, res, post_handlers_);
		post_handler_(req, res);
		return true;
    }
    return false;
}

//inline bool HttpServer::dispatch_request(Request& req, Response& res, Handlers& handlers)
//{
...
inline void HttpServer::process_request(Stream& strm)
{
    Request req;
    Response res;
    ...
    if (!read_request_line(strm, req) ||
        ...
        // TODO:
		M_HTTP_PRINTF("process_request__E1..\r\n");
        return;
    }

    if (req.method == "POST") {
        if (!detail::read_content(strm, req, false)) {
			M_HTTP_PRINTF("process_request__E2..\r\n");
            // TODO:
            return;
        }
...
inline bool HttpServer::read_and_close_socket(socket_t sock)
{
    return detail::read_and_close_socket(sock, [this](Stream& strm) {
		M_HTTP_PRINTF("process request..\r\n");
...
inline HttpClient::HttpClient(const char* host, int port)
    : host_(host)
    , port_(port)
{
}
...
inline bool HttpClient::process_request(Stream& strm, const Request& req, Response& res)
{
    // Send request
...
inline bool HttpClient::read_and_close_socket(socket_t sock, const Request& req, Response& res)
{
...
inline void HttpClient::add_default_headers(Request& req)
{
...
inline std::shared_ptr<Response> HttpClient::post(
...
inline bool read_and_close_socket_ssl(socket_t sock, SSL_CTX* ctx, U SSL_connect_or_accept, V setup, T callback)
{
	...
	//M_HTTP_PRINTF("read_and_close_socket_ssl_1..\r\n");
    auto bio = BIO_new_socket(sock, BIO_NOCLOSE);
    SSL_set_bio(ssl, bio, bio);

    setup(ssl);
	//M_HTTP_PRINTF("read_and_close_socket_ssl_2..\r\n");
    SSL_connect_or_accept(ssl);
	//M_HTTP_PRINTF("read_and_close_socket_ssl_3..\r\n");
    SSLSocketStream strm(ssl);
    auto ret = callback(strm);
	//M_HTTP_PRINTF("read_and_close_socket_ssl_4..\r\n");
    SSL_shutdown(ssl);
    SSL_free(ssl);
    close_socket(sock);
    return ret;
...
inline SSLHttpServer::SSLHttpServer(const char* cert_path, const char* private_key_path)
{
    ...
    if (ctx_) {
        ...
        if (SSL_CTX_use_certificate_file(ctx_, cert_path, SSL_FILETYPE_PEM) != 1 ||
            ...
            SSL_CTX_free(ctx_);
            ctx_ = nullptr;
			//M_HTTP_PRINTF("SSL Use Certfailed..\r\n");
        }
	}
	else {
		//M_HTTP_PRINTF("SSL_CTX_new failed..\r\n");
	}
}

inline SSLHttpServer::~SSLHttpServer()
{
    if (ctx_) {
        SSL_CTX_free(ctx_);
    }
...
inline bool SSLHttpServer::read_and_close_socket(socket_t sock)
{
...
void set_response_post(httplib::Response &res, std::string &str, int sts = 200);
void set_response_buf(httplib::Response &res, const char* buf, int len,int sts=200);
