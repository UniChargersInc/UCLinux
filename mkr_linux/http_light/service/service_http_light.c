...
extern int http_light_main_run;
...
void __stdcall service_main(DWORD dwArgc, LPTSTR *lpszArgv)
{
	...
	int rc;
	service_handle = RegisterServiceCtrlHandler(TEXT("mkr_http_server"), service_handler);
	if(service_handle){
...
void service_install(void)
{
	...
	TCHAR exe_path[MAX_PATH + 5];
	SERVICE_DESCRIPTION svc_desc;

	GetModuleFileName(NULL, exe_path, MAX_PATH);
	wcscat(exe_path,TEXT (" run")); //strcat(exe_path, " run");

	sc_manager = OpenSCManager(NULL, NULL, SC_MANAGER_CREATE_SERVICE);
	if(sc_manager){
		svc_handle = CreateService(sc_manager, TEXT ("mkr_http_server"), TEXT("Mikerel Server"),
				SERVICE_START | SERVICE_STOP | SERVICE_CHANGE_CONFIG,
				SERVICE_WIN32_OWN_PROCESS, SERVICE_AUTO_START, SERVICE_ERROR_NORMAL,
				exe_path, NULL, NULL, NULL, NULL, NULL);

		if(svc_handle){
			svc_desc.lpDescription = TEXT("Mikerel Serverr v.1.01");
...
void service_uninstall(void)
{
	...
	if(sc_manager){
		svc_handle = OpenService(sc_manager,TEXT("mkr_http_server"), SERVICE_QUERY_STATUS | DELETE);
		if(svc_handle){
